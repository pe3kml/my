...
public class fcie {
	...
	public static void searchClient() {
		...
		String sql = new String(
				"SELECT c.id_client,c.name_c,c.surname FROM Client c WHERE CAST(c.id_client AS TEXT) LIKE '"
						+ b + "%' and  c.surname LIKE '"
						+ a + "%' ORDER BY c.surname ASC;");
		ResultSet result = API.SelectFromDb.select(sql);
		GUI.Grafika.table_2.setModel(DbUtils.resultSetToTableModel(result));
	...
	public static void Statistic() {
		...
		GUI.Grafika.TAStatistic.append("  Poèet pacientov doktorov: \n");
		try {
			while (result.next()) {
				GUI.Grafika.TAStatistic.append("    " + result.getString(2)
						+ " - " + result.getString(1) + "\n");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		...
		try {
			while (result.next()) {
				GUI.Grafika.TAStatistic
						.append(result.getString(1) + " rokov\n");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
		...
		try {
			while (result.next()) {
				GUI.Grafika.TAStatistic.append("    od " + result.getString(1)
		...
		try {
			while (result.next()) {
				GUI.Grafika.TAStatistic.append("    " + result.getString(1)
						+ " - " + result.getString(2) + "\n");
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
